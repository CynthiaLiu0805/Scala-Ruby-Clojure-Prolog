...
% when substitution exists, call helper function to simplify the substitution, then put the result into interpretVarExpr predicate
...
% subst(subst(plusE(var(x),var(y)),x,constE(5)), y,constE(4)),9).
...
subhelp(plusE(A,B),C,D,R) :- A==var(C) -> R = plusE(D,B), write(R).
